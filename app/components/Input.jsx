 ...
 * }>}
 */
const Input = ({ id, name, label, className, classes = {}, type = 'text', options = [], errors = [], ...attrs }) => {
	...
	if (['radio', 'checkbox'].includes(type)) {
		sharedAttrs.className += ' relative w-4 h-4 mr-1 appearance-none checked:bg-primary checked:bg-check-white';
	} else {
	...
	return (
		<div
			className={['app-input', errors.length && 'app-input--error', ['radio', 'checkbox'].includes(type) ? 'flex items-center' : '', className, classes.root]
				.filter(Boolean)
				.join(' ')}
		>
			{type !== 'checkbox' && (
			...
			{type === 'select' && (
				<select {...sharedAttrs} {...attrs} className={[sharedAttrs.className, classes.input].filter(Boolean).join(' ')}>
			...
			{type !== 'select' && <input {...sharedAttrs} type={type} {...attrs} className={[sharedAttrs.className, classes.input].filter(Boolean).join(' ')} />}
